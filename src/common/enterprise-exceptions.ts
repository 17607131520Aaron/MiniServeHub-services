...
import { APP_ERROR_CODES, AppHttpException } from '../common/common-errors';

/**
 * AppValidationException 类表示应用程序验证异常
 * 当输入数据不符合验证规则时抛出此异常
 * @extends AppHttpException - 继承自应用程序HTTP异常基类
 */
export class AppValidationException extends AppHttpException {
  /**
   * 创建 AppValidationException 实例
   * @param message - 可选的错误描述信息，默认为验证错误的标准消息
   * @param status - HTTP状态码，默认为 HttpStatus.BAD_REQUEST (400)
   */
